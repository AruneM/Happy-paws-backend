        ...
        .then((passwordHash) => {
          UserModel.create({fullName, email, passwordHash, phone, location, job, livingPlace, otherowned, availability })
            .then((adopter) => {
              ...
              req.session.loggedInAdopt = adopter;
              console.log(req.session)
  ...
  const {email, password } = req.body;
    ...
    UserModel.findOne({email})
      .then((adopterData) => {
          ...
          bcrypt.compare(password, adopterData.passwordHash)
            .then((doesItMatch) => {
                //if it matches
                if (doesItMatch) {
                  // req.session is the special object that is available to you
                  adopterData.passwordHash = "***";
                  req.session.loggedInAdopt = adopterData;
                  console.log('Signin', req.session)
                  res.status(200).json(adopterData)
                }
...
router.get("/adopter", isLoggedInAdopter, (req, res, next) => {
  res.status(200).json(req.session.loggedInAdopt);
});
